...
### 4. 대시보드 확인
- 백엔드 실행 후 브라우저에서 `http://<host>:8000/dashboard/` 에 접속하면 **BITWatch AI 서버 관리 시스템** UI에서 최신 메트릭과 수집된 로그를 확인할 수 있습니다.
...
- 실시간 네트워크 대역폭 수집에는 `iftop`이 권장되며, 미설치 또는 권한 부족 시 psutil 기반 추정값으로 대체됩니다. `iftop` 실행에는 루트 권한이 필요할 수 있습니다.
- GPU 자원 모니터링은 NVIDIA 드라이버 및 `nvidia-smi` CLI가 필요합니다. GPU가 존재하지 않거나 권한이 없다면 대시보드에는 “GPU 없음”으로 표시됩니다.
